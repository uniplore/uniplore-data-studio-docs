...
2. 执行`yarn serve`或`npm run serve`启动前端项目；
3. 访问`http://localhost:3000`;

## 答疑

为什么只需要创建数据库即可？库表是如何生成的？本项目使用的是 Flyway 作为数据库版本管理工具，在启动项目时会自动执行数据库脚本，生成库表。
...
| uniplore-flink-chunjun-go   | 基于 golang 调用 flink(chunjun)提交任务 | 0.1.0    |   应用模块   | 解决java服务调用Flink提交任务能力。不直接调用 Flink RestAPI? 因为本项目集成了[Chunjun](https://dtstack.github.io/chunjun/) 提供数据同步支持 |
